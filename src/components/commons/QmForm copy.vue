<template>
    <div class="filter-form detail-form">
        <div class="filter-form-item" v-for="(item, index) in formConfig" :key="index">
            ...
            <!-- 静态带list的select -->
            <el-select v-else-if="item.type === 'select' && item.list" v-model="formClone[item.key]" @change="modelHandle"
                clearable :placeholder="item.title">
            ...
            <el-cascader :popper-class="'el-cascader-' + item.key" style="min-width: 300px"
                v-else-if="item.type === 'cascaderTreeOrg'" @change="cascaderChange($event, item, selectObj[item.key])"
                v-model="formClone[item.key]" clearable :collapse-tags="true" :options="selectObj[item.key]" :props="{
                    label: item.label,
                    value: item.value,
            ...
            <!-- 输入框 -->
            <el-input v-else-if="item.type === 'search'" prefix-icon="el-icon-search" v-model="formClone[item.key]"
                clearable @change="modelHandle" placeholder="请输入关键词">
        ...
        <div class="total">
            <slot name="total">
                <template v-if="total || total === 0"> 设备查询结果：{{ total }} </template>
            </slot>
        </div>
    </div>
</template>
